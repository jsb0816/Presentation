    ...
    return (
        <div className="w-[1280px] h-[720px] bg-white text-slate-900 p-16 flex flex-col justify-between border-8 border-slate-100 shadow-2xl">
            ...
            <div className="border-b border-slate-200 pb-6">
                ...
                <h1 className="text-5xl font-black italic text-slate-900">How It Works: <span className="text-indigo-600 font-normal">AI & Data Logic</span></h1>
            </div>
                    ...
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 shadow-sm">
                        <ul className="space-y-4 text-slate-600">
                            <li className="flex gap-3">
                                ...
                                <span><strong className="text-slate-900">시공간 맥락 분석:</strong> 단일 프레임이 아닌 프레임 간의 흐름을 파악하여 행동 의도 파악</span>
                            </li>
                            <li className="flex gap-3">
                                <Zap className="text-yellow-500 shrink-0" size={20} />
                                <span><strong className="text-slate-900">Shifted Window Attention:</strong> 연산 효율을 극대화하여 모바일 환경에서도 실시간성 유지</span>
                            </li>
                            <li className="flex gap-3">
                                <Zap className="text-yellow-500 shrink-0" size={20} />
                                <span><strong className="text-slate-900">AI Hub 데이터 학습:</strong> 운전자 부주의 상황(졸음/폭행/휴대폰)에 최적화된 가중치 적용</span>
                            </li>
                        </ul>
                    </div>
                </section>
                    ...
                    <div className="relative">
                        ...
                        <div className="bg-indigo-50/50 border-2 border-dashed border-indigo-200 p-8 rounded-3xl text-center">
                            <div className="flex justify-around items-center mb-6 text-slate-700">
                                <div className="text-xs">GPS & T-Map<br /><span className="text-lg font-bold text-slate-900">도로 규정</span></div>
                                <div className="text-2xl font-light text-slate-400">+</div>
                                <div className="text-xs">IMU Sensor<br /><span className="text-lg font-bold text-slate-900">G-Force</span></div>
                                <div className="text-2xl font-light text-slate-400">+</div>
                                <div className="text-xs">AI Inference<br /><span className="text-lg font-bold text-slate-900">상태 코드</span></div>
                            </div>

                            <div className="bg-indigo-600 text-white py-4 rounded-xl text-2xl font-black tracking-tighter shadow-lg shadow-indigo-500/30">
            ...
            <div className="flex gap-4">
                {['PyTorch', 'FastAPI', 'Vercel', 'AWS EC2', 'T-Map API'].map(tech => (
                    <span key={tech} className="px-4 py-1 bg-white rounded-full text-xs font-mono text-slate-500 border border-slate-200 shadow-sm">
                        {tech}
                    </span>
                ))}
            </div>
