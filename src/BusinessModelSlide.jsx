...
const BusinessModelSlide = () => {
    return (
        <div className="w-[1280px] h-[720px] bg-white text-slate-900 p-16 flex flex-col justify-between overflow-hidden relative shadow-2xl border-8 border-slate-100">
            ...
            {/* 하단 요약 슬로건 */}
            <div className="relative z-10 bg-gradient-to-r from-emerald-50 to-blue-50 border border-slate-200 p-10 rounded-full flex items-center justify-between shadow-sm">
...
const BMLogicCard = ({ icon, actor, shortTerm, impact }) => (
    <div className="bg-slate-50 border border-slate-200 p-8 rounded-[40px] hover:bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
        ...
        <h3 className="text-2xl font-black mb-4 text-slate-900 group-hover:text-emerald-600 transition-colors">{actor}</h3>
        <div className="space-y-4">
            <div>
                <p className="text-[10px] uppercase text-slate-400 font-bold mb-1">Incentive</p>
                <p className="text-sm font-medium text-slate-700">{shortTerm}</p>
            </div>
            <div className="pt-4 border-t border-slate-200">
                <p className="text-[10px] uppercase text-slate-400 font-bold mb-1">Social Value</p>
                <p className="text-sm font-medium text-slate-500 italic">"{impact}"</p>
            </div>
        </div>
    </div>
);
